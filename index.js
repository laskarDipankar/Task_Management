...
app.get("/api/users/:id", (req, res) => {
  User.find({ _id: req.params.id }).exec((error, result) => {
    if (error) {
      res.status(400).send({
      ...
      if (taskResult == null) {
        ...
        if (isComplete == true) {
          ...
          if (updatedTask == null) {
            ...
            res
              .status(200)
              .send({ message: "user is assigned tasks", data: updatedTask });
          } // res.send(UpdateUser)
        } else {
          res.send({
...
app.delete("/api/users/:id", async (req, res) => {
  ...
  if (result == null) {
    ...
    const DelUserDetails = await Task.findOneAndUpdate(
      { assignedUser: id },
      { $set: { completed: true, assignedUser: "", assignedUserName: "" } }
    );
...
app.delete("/api/tasks/:id", async (req, res) => {
  ...
  if (result == null) {
    ...
    if (result.assignedUser != "" && !result.completed) {
      ...
      console.log(data.pendingTasks.length);
